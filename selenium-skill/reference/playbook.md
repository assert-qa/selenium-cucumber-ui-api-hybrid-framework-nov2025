...
## §16 — API Integration/Service Layer

```java
public class UserService {
    public User getUserById(int id) {
        Response res = RestAssured.get("/api/users/" + id);
...
## §17 — Advanced Test Data Management
...
@BeforeEach
void seedData() {
    // Call API to reset DB or seed test data
    RestAssured.post("/api/test/reset");
}
```
...
// Load profile by system property
-Denv=qa

// Java
String env = System.getProperty("env", "default");
String configFile = "config" + (env.equals("default") ? "" : ("-" + env)) + ".properties";
```

---

## §19 — Suite-Level Parallelization & Cloud Grid

### Selenium Grid
```shell
# Start hub & nodes, then run tests with
-Dgrid.url=http://localhost:4444/wd/hub
```

### LambdaTest/BrowserStack Example
```java
String hub = "https://USERNAME:KEY@hub.lambdatest.com/wd/hub";
WebDriver driver = new RemoteWebDriver(new URL(hub), caps);
```

---

## §20 — Flakiness Analysis & Rerun
...
assertTrue(violations.length() == 0, "Accessibility issues found");
```

---

## §22 — Security & Performance Hooks
...
void testCSPHeader() {
    ...
    String csp = driver.manage().getCookieNamed("Content-Security-Policy").getValue();
    assertNotNull(csp);
}
```
...
## §23 — Test Maintainability Metrics

- Gunakan plugin SonarQube/Jacoco untuk coverage.
- Analisis duplikasi test dan unused page object dengan static analysis.
- Lakukan code review berkala untuk menghapus test yang obsolete.

---

## §24 — Migration/Upgrade Tips

- Selalu cek [Selenium changelog](https://github.com/SeleniumHQ/selenium/blob/trunk/java/CHANGELOG)
- Gunakan WebDriverManager versi terbaru untuk auto-update driver
- Upgrade Allure/ExtentReports secara berkala, cek breaking changes
- Jalankan test di branch khusus sebelum merge ke main setelah upgrade
- Simpan backup config lama sebelum migrasi
